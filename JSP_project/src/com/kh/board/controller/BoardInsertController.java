...
//			String title= request.getParameter("title"); //null
			...
			if(ServletFileUpload.isMultipartContent(request)) {
				...
				// 1. 전송되는 파일을 처리할 작업내용 (전송되는 파일의 용량 제한, 전달된 파일을 저장할 경로) 
				// 1_1. 전송파일 용량제한(int maxSize => byte단위로 값을 기술해야함.) 저희는 10Mbyte로 제한할 예정 
				/*
				 * 단위정리
				 * 1byte -> kbyte -> mbyte -> gbyte -> tbyte -> ...
				 * 1kbyte -> 1024byte (2의 10승)
				 * 1mbyte -> 1024kbyte -> 1024 * 1024byte -> (2의 20승)
				 * 
				 */
				 ...
				 * 다만 WebContent 폴더로부터 board_upfiles 폴더까지의 경로는 매개변수로 제시해줘야 한다. 
				 * 그리고 결론적으로는 board_upfiles 폴더내부에 파일들이 저장될 것이기 때문에 /를 마지막에 붙여줘야 한다. 
				 * 
				 * */
				 ...
				 *  MultipartRequest multi = new MultipartRequest(request 객체, 저장할 폴더 경로, 용량제한, 인코딩값, 
				 *    파일명을 수정시켜주는 객체);
				 *  
				 *  위 구문 한줄 실행만으로 넘어온 첨부파일들이 해당 폴더에 업로드됨 
				 *  그리고 사용자가 올린 파일명은 그대로 해당폴더에 업로드하지 않는게 관례 
				 *  - 같은 파일명이 있을 경우 덮어씌워지거나 에러가 날 수 있고, 한글/특수문자/띄어쓰기가 포함된 파일명일 경우 사용하는
				 *  서버에 따라 에러가 발생할 수 있기 때문. 따라서 파일명은 반드시 변환시켜줘서 업로드할 예정 
				 *  
				 ...
				 *  
				 *  - 절대 파일명이 겹치지 않게끔 rename해볼예정 
				 *  - 즉, DefaultFilenamePolicy 객체는 사용하지 않음 
				...
				// 3. DB에 기록할 데이터들을 뽑아서 Attachment 객체에 담기, Board객체에 각각 담아주기  
				...
				if( result > 0) { // 성공적으로 작성 -> 최신글 목록으로 이동 
					...
					if(at != null) {
						// 삭제시키고자 하는 파일객체 생성 후 delete 메소드 호출시 파일이 삭제된다. 
