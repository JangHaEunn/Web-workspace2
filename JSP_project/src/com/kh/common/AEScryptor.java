...
//javax.crypto, java.security 패키지에 구성되어 있음 
public class AEScryptor {
	// 저희만의 key를 만들예정. key는 프로젝트 당 한개만 있어야 하며 보안이 굉장히 중요함.
	
	private static SecretKey key; // 암호화를 위한 키
	private String path; // 파일로 저장된 암호화키의 위치 
	
	public AEScryptor() {
		// 클래스가 생성될 떄 기본설정을 해줘야함. 
		// 1. key파일에 객체가 생성되어 있다면 key파일에서 SecretKey 객체를 불러오고 
		// 생성되어 있지 않다면, SecretKey 객체를 생성하여 파일로 저장시킨다. 
		...
		// key를 저장하고 있는 파일이름이 bsl.bs -> SecretKey 객체가 내장되어 있음.
	...
	// 암호화된 데이터를 다시한번 복호화(원본값으로 변경)하는 메소드 -> key가 필요함
