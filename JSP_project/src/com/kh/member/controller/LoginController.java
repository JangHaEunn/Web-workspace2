...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * - POST 방식 : 사용자가 입력한 값들이 url 노출 X/ 데이터 길이제한 X/ 대신 Timeout이 존재 
		 * 
		 */
		...
		if(loginUser == null) { // 로그인 실패 => 에러페이지 응답 
			...
			// Servlet에서 JSP내장객체인 SESSION에 접근하고자 한다면 우선 session 객체를 얻어와야 함. 
			...
			// 	  url에 내용이 그대로 남아있는 상태에서 새로고침(f5) 누르게되면 계속 로그인 요청을 보내게 될것.
