...
<!-- 
	변수명만 제시하면 공유범위가 가장 작은곳에서부터 큰곳으로 순차적으로 찾아지게 됨. 티가나지는 않지만 속도가 조금 느려질수가 있다.
...
<c:out value="${outTest }"/> <br> <!-- escapeXml 생략시 기본값은 true == 태그해석안됨(문자열로 취급함) -->
...
<%
	...
	list.add(new Person("민","여","파일럿"));
	list.add(new Person("경민","남","간호사"));
...
<pre>
&lt;c:forTokens var="각 값을 보관할 변수" items="분리시키고자 하는 문자열" delims="구분자"&gt;
...
&lt;c:url var="변수명" value="요청할url"&gt;
	...
	&lt;c:param name="키값" value="밸류값"/&gt;
&lt;/c:url&gt;
- url경로를 생성을하고, 쿼리스트링을 정의할 수 있는 태그 
- 넘겨줘야할 쿼리스트링이 길 경우 사용하면 편리하고,
- 특정조건에 따라 파라미터값을 다르게 보내야 할때 자주 사용됨 
</pre>
