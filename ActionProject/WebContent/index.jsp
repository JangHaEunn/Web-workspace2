...
<body>
	...
	<p>
		...
		JSP상에서 표현하고자 하는 값을 \${name}<%--  == <%=name %> --%>의 형식으로 표현해서 작성하는 것.
	...
	<!-- 
		* JSP를 이루는 구성인자
		...
		1. JSP 스크립팅원소 : JSP페이지 안에서 자바코드를 직접 기술할 수 있게 하는 기술
			EX) 선언문, 스크립틀릿, 표현식, 지시어.. 
		
		2. 지시어 : JSP페이지 정보에 대한 내용을 표현한다거나 또다른 페이지를 포함할때 사용
		    EX) 지시어 (page 지시어, include 지시어, taglib 지시어(라이브러리 추가시 사용) 
		    
		3. JSP 액션 태그 : XML기술을 이용해서 기존 jsp문법을 확장하는 기술을 제공하는 태그 
		        - 표준 액션 태그 (Standard Action Tag) : JSP페이지에서 바로 사용가능(별도의 라이브러리가 필요 없다)
		                                              표준액션태그는 모든 태그명 앞에 jsp: 이라는 접두어가 붙음. 
		        - 커스텀액션태그(Custom Action Tag)    : JSP페이지에서 바로 사용 불가능(별도의 라이브러리 연동 해줘야함)
		                                              커스텀 액션태그는 모든 태그명 안에 별도의 접두어가 붙음(c:, fn:, fmt:)
